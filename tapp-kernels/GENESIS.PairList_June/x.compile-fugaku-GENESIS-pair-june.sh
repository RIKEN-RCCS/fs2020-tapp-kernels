...
TMPDIR=${HOME}/tmp/check_GENESIS-pairlist-june
mkdir -p ${TMPDIR}
cd ${TMPDIR}/
if [ $? != 0 ] ; then echo '@@@ Directory error @@@'; exit; fi
rm *

SRC_DIR=${HOME}/fs2020_kernels/src/GENESIS.PairList_June
cp -rp ${SRC_DIR}/* ./

gunzip data_pairlist_june.gz
...
frt -c ${FOPTIMIZE}  kernel_pairlist_june.f90
frt -c ${FOPTIMIZE}  sub_data_io.f90
#	fcc -c ${COPTIMIZE} common/src/report.c

frt ${FOPTIMIZE} initialize_data.o main.o kernel_pairlist_june.o sub_data_io.o
